...
// Type imports
type UniqueIdentifier = DndCore.UniqueIdentifier;
type DraggableAttributes = DndCore.DraggableAttributes;
type DraggableSyntheticListeners = DndCore.DraggableSyntheticListeners;
...
const dropAnimationConfig: DndCore.DropAnimation = {
  ...DndCore.defaultDropAnimation,
  sideEffects: DndCore.defaultDropAnimationSideEffects({
    styles: {
      active: {
        opacity: '0.4',
      },
...
function Kanban<T>({ value, onValueChange, getItemValue, children, className, onMove }: KanbanRootProps<T>) {
  ...
  const handleDragStart = React.useCallback((event: DndCore.DragStartEvent) => {
    setActiveId(event.active.id);
  }, []);

  const handleDragOver = React.useCallback(
    (event: DndCore.DragOverEvent) => {
      if (onMove) {
        return;
      }
    ...
    (event: DndCore.DragEndEvent) => {
      ...
      if (isColumn(active.id) && isColumn(over.id)) {
        ...
        if (activeIndex !== overIndex) {
          const newOrder = DndSortable.arrayMove(Object.keys(columns), activeIndex, overIndex);
      ...
      if (activeContainer && overContainer && activeContainer === overContainer) {
        ...
        if (activeIndex !== overIndex) {
          setColumns({
            ...columns,
            [container]: DndSortable.arrayMove(columns[container], activeIndex, overIndex),
          });
        }
      }
    },
  ...
  return (
    <KanbanContext.Provider value={contextValue}>
      <DndCore.DndContext sensors={sensors} onDragStart={handleDragStart} onDragOver={handleDragOver} onDragEnd={handleDragEnd}>
...
function KanbanBoard({ children, className }: KanbanBoardProps) {
  ...
  return (
    <DndSortable.SortableContext items={columnIds} strategy={DndSortable.rectSortingStrategy}>
...
function KanbanColumnContent({ value, className, children }: KanbanColumnContentProps) {
  ...
  return (
    <DndSortable.SortableContext items={itemIds} strategy={DndSortable.verticalListSortingStrategy}>
...
function KanbanOverlay({ children, className }: KanbanOverlayProps) {
  ...
  return (
    <DndCore.DragOverlay dropAnimation={dropAnimationConfig}>
      <div
